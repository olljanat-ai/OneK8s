...
variable "enable_observability" {
  description = "Install the Grafana k8s-monitoring collectors and ship this cluster's metrics, logs and events to Grafana Cloud. Off by default in this lab; it needs the credentials in the OCI Vault under var.grafana_cloud_secret_name, and the endpoints come from modules/platform-observability."
  type        = bool
  default     = false
}
